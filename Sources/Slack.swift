//
//  SwiftBeaverSlack.swift
//  MPEX
//
//  Created by hoangvannghia on 12/24/17.
//  Copyright © 2017 hoang van nghia. All rights reserved.
//
...
public class SwiftBeaverSlack: BaseDestination {
   ...
   public init(slackWebHook: String,
               entriesFileName: String = "swiftbeaver_slack_entries.json",
               sendingfileName: String = "swiftbeaver_slack_entries_sending.json",
               analyticsFileName: String = "swiftbeaver_slack_analytics.json") {
      super.init()
   ...
   public func sendNow() {
      ...
      if !sendingInProgress {
         ...
         if lines > 0 {
            sendToServerAsync(logEntries, complete: { (ok, _) in
   ...
   func sendToServerAsync(_ str: [[String: Any]], complete: @escaping (_ ok: Bool, _ status: Int) -> Void) {
      ...
      for i in 0...str.count / 20 {
         ...
         payload["attachments"] = str[begin...end].map({ entry -> [String: Any] in
            let level = SwiftyBeaver.Level(rawValue: entry["level"] as? Int ?? 0) ?? .warning
            let fileInfo = (entry["fileName"] as? String ?? "") + "." + (entry["function"] as? String ?? "")
               + ":\(entry["line"] as? Int ?? 0)"
            
            
            let deviceInfo = ["appVersion", "os", "osVersion", "deviceName", "deviceModel"].map({
               return analyticsDict[$0] as? String ?? ""
            }).joined(separator: " ")
            
            
            return ["fallback": level.fallbackMessage,
                    "color": level.color,
                    "author_name": analyticsDict["userName"] as? String,
                    "pretext": fileInfo,
                    "title": level.fallbackMessage, //fileInfo,
               "ts": entry["timestamp"],
               "text": entry["message"],
               "footer": deviceInfo,
         ...
         // POST parameters
         request.httpBody = jsonStringFromDict(payload)?.data(using: .utf8) ?? Data()
         toNSLog("sending ...")
